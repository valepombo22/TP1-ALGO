...
/* Función auxiliar para reemplazar strdup */
...
static enum tipo_pokemon tipo_desde_string(const char *str) {
    if (strcmp(str, "ELEC") == 0) return TIPO_ELEC;
    if (strcmp(str, "FUEG") == 0) return TIPO_FUEG;
    if (strcmp(str, "PLAN") == 0) return TIPO_PLAN;
    if (strcmp(str, "AGUA") == 0) return TIPO_AGUA;
    if (strcmp(str, "NORM") == 0) return TIPO_NORM;
    if (strcmp(str, "FANT") == 0) return TIPO_FANT;
    if (strcmp(str, "PSI") == 0) return TIPO_PSI;
    if (strcmp(str, "LUCH") == 0) return TIPO_LUCH;
    return TIPO_NORM;
}
...
tp1_t *tp1_leer_archivo(const char *nombre_archivo) {
    ...
    while (fgets(linea, sizeof(linea), f)) {
        ...
        int leidos = sscanf(linea, "%d,%255[^,],%15[^,],%d,%d,%d",
                            &id, nombre, tipo_str, &ataque, &defensa, &velocidad);

        bool valido = (leidos == 6) && !existe_id(tp, id);
        if (valido) {
            if (tp->cantidad == tp->capacidad) {
                size_t nueva_capacidad = (tp->capacidad == 0) ? 4 : tp->capacidad * 2;
...
tp1_t *tp1_guardar_archivo(tp1_t *tp, const char *nombre_archivo) {
    ...
    for (i = 0; i < tp->cantidad; i++) {
        ...
        const char *tipo_str;
        if (p->tipo == TIPO_ELEC) tipo_str = "ELEC";
        else if (p->tipo == TIPO_FUEG) tipo_str = "FUEG";
        else if (p->tipo == TIPO_PLAN) tipo_str = "PLAN";
        else if (p->tipo == TIPO_AGUA) tipo_str = "AGUA";
        else if (p->tipo == TIPO_NORM) tipo_str = "NORM";
        else if (p->tipo == TIPO_FANT) tipo_str = "FANT";
        else if (p->tipo == TIPO_PSI) tipo_str = "PSI";
        else if (p->tipo == TIPO_LUCH) tipo_str = "LUCH";
        else tipo_str = "NORM";
...
tp1_t *tp1_union(tp1_t *un_tp, tp1_t *otro_tp) {
    ...
    for (i = 0; i < un_tp->cantidad; i++) {
        if (nuevo->cantidad == nuevo->capacidad) {
            size_t nueva_cap = (nuevo->capacidad == 0) ? 4 : nuevo->capacidad * 2;
            struct pokemon *aux = realloc(nuevo->pokemones, nueva_cap * sizeof(struct pokemon));
            if (!aux) { tp1_destruir(nuevo); return NULL; }
            nuevo->pokemones = aux;
...
size_t tp1_con_cada_pokemon(tp1_t *tp, bool (*f)(struct pokemon *, void *), void *extra) {
    if (!tp || !f) return 0;
    size_t cont = 0;
    size_t i;
    for (i = 0; i < tp->cantidad; i++) {

...
void prueba_buscar_id() {
    ...
    pa2m_afirmar(pk != NULL && strcmp(pk->nombre, "Charmander") == 0, "Buscar por ID funciona");
    tp1_destruir(tp);
}
...
void prueba_union() {
    ...
    pa2m_afirmar(res != NULL && tp1_cantidad(res) >= tp1_cantidad(tp1), "Union de pokedex funciona");
    tp1_destruir(tp1);
    tp1_destruir(tp2);
    tp1_destruir(res);
}
